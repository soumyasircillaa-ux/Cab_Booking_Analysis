/*
Project Title : Deign RDBMS for Drive_NXT_Cab_Bookings

Objective : 
1. Design RDBMS schema for Drive_NXT_Cab_Bookings and Understand the trend analysis 
2. Understand operational performance of Drive_NXT_Cab_Bookings
...
Create Database = Drive_NXT_Cab_Bookings

*/

Create Database if not exists Drive_NXT_Cab_Bookings;
Use Drive_NXT_Cab_Bookings;
...
Booking_Status ENUM ('Pending', 'Confirmed', 'InProgress',
 'Completed', 'CancelledByCustomer', 'CancelledByDriver'),
...
Payment_Type ENUM('Cash', 'Card', 'Wallet', 'UPI') 
DEFAULT 'Cash',
Customer_ID INT NOT NULL,
Driver_ID INT NOT NULL,
Cab_ID INT NOT NULL,
...
SELECT 
    ...
    SUM(CASE WHEN b.Booking_Status IN 
    ('CancelledByCustomer', 'CancelledByDriver') THEN 1 ELSE 0 END) 
    AS Cancelled_Bookings,
    ROUND(
        SUM(CASE WHEN b.Booking_Status IN 
        ('CancelledByCustomer', 'CancelledByDriver') 
        THEN 1 ELSE 0 END) * 100.0 / COUNT(b.Booking_ID), 
        2
...
SELECT 
    ...
    SUM(CASE WHEN Booking_Status = 
    'Completed' THEN 1 ELSE 0 END) AS Completed_Bookings,
    SUM(CASE WHEN Booking_Status 
    IN ('CancelledByCustomer', 'CancelledByDriver') THEN 1 ELSE 0 END)
    AS Cancelled_Bookings,
ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM Booking_Info), 2) 
...
GROUP BY d.Driver_ID, d.Driver_Name,
 d.Driver_Age, d.Driver_JoinDate
...
GROUP BY d.Driver_ID , d.Driver_Name ,
 d.Driver_Age , d.Driver_Location
...
SELECT 
    ...
    d.Driver_Location,
    COUNT(b.Booking_ID) 
    AS Total_Assigned_Bookings,
    SUM(CASE WHEN b.Booking_Status = 
    'CancelledByDriver' THEN 1 ELSE 0 END) 
    AS Driver_Cancellations,
    ROUND(SUM(CASE WHEN b.Booking_Status = 
    'CancelledByDriver' THEN 1 ELSE 0 END) 
    * 100.0 / COUNT(b.Booking_ID), 2)
    AS Cancellation_Percentage,
    SUM(CASE WHEN b.Booking_Status =
    'Completed' THEN 1 ELSE 0 END) AS Completed_Trips
FROM Drivers_Info d
...
SELECT 
    SUM(CASE WHEN Trip_Distance_KM <= 8 THEN Final_Amount ELSE 0 END) 
    AS Revenue_From_Short_Trips,
    SUM(Final_Amount) AS Total_Revenue,
    ROUND(SUM(CASE WHEN Trip_Distance_KM <= 8 
    THEN Final_Amount ELSE 0 END) * 100.0 / SUM(Final_Amount), 1) 
    AS Short_Trip_Revenue_Percentage
FROM Trip_Details t
JOIN Booking_Info b ON t.Booking_ID = b.Booking_ID
WHERE b.Booking_Status = 'Completed';
...
- The company should NOT encourage more short-distance rides. Instead, focus resources
on promoting medium and long-distance trips (8-25 km) where 97.3% of revenue comes from.
*/
...
SELECT 
    ...
    ROUND(COUNT(b.Booking_ID) / COUNT(DISTINCT DATE(b.Booking_Time)), 1) 
    AS Trips_Per_Day
FROM Drivers_Info d
JOIN Booking_Info b ON d.Driver_ID = b.Driver_ID
WHERE b.Booking_Status = 'Completed'
...
SELECT 
    ...
    ROUND(SUM(t.Final_Amount) / NULLIF(SUM(t.Trip_Distance_KM), 0), 2)
    AS Revenue_Per_KM
FROM Trip_Details t
JOIN Booking_Info b ON t.Booking_ID = b.Booking_ID
...
SELECT 
    ...
    DATEDIFF(MAX(b.Booking_Time), MIN(b.Booking_Time)) 
